...
int main () {
	...
	for(int i = 1; i < 11; i++){
		...
		for (int x =0 ; x < scene.getWidth() ; x++) {	
			for (int  y = 0 ; y < scene.getHeight() ; y++) {
				Job * j = new DrawJob(x,y,scene,screen,pixels,lights);
				pool.submit(j);
			}
		}
		pool.stop();
	    ...
	    std::cout << "Total time ";
		time_t time = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
	              cout << time
	              << "ms.\n";
	...
	exemple d'une execution:
	 NBTHREADS = 1 Total time 42635ms.
	NBTHREADS = 2 Total time 97967ms.
	NBTHREADS = 3 Total time 154041ms.
	NBTHREADS = 4 Total time 211649ms.
	NBTHREADS = 5 Total time 270214ms.
	NBTHREADS = 6 Total time 328553ms.
	NBTHREADS = 7 Total time 381163ms.
	NBTHREADS = 8 Total time 402975ms.
	NBTHREADS = 9 Total time 418640ms.
	NBTHREADS = 10 Total time 437188ms.
	nb de thread associé au temps min = 1
	*/

	exportImage("toto.ppm",scene_min.getWidth(), scene_min.getHeight() , pixels_min);
